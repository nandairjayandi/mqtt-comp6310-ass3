...
static void run_burst(void) {
    ...
    char topic[64];
    snprintf(topic, sizeof(topic), "counter/%d/%d/%d",
    ...
    long long counter  = 0;
    FILE     *tsv      = open_sent_tsv(counter);
    if (tsv == NULL) {
        free(x_str);
        free(payload);
        return;
    ...
    while (time(NULL) < end_time) {
        ...
        int payload_len;
        if (msg_size > 0) {
            payload_len = snprintf(payload, payload_buf_size,
                                   "%lld:%lld:%s", counter, pub_ts, x_str);
        } else {
            payload_len = snprintf(payload, payload_buf_size,
                                   "%lld:%lld:", counter, pub_ts);
        }

        /* Publish — only write TSV row on success */
        int rc = MQTTClient_publish(g_client, topic, payload_len,
                                    payload, qos, 0, NULL);
...
static int on_message(void *context,
    ...
    /* Safe null-terminated payload copy (config values are short) */
    char payload[64];
    ...
    if (strcmp(topic, "request/qos") == 0) {
        cfg_qos = atoi(payload);
        printf("publisher: config — qos=%d\n", cfg_qos);

    } else if (strcmp(topic, "request/delay") == 0) {
        cfg_delay_ms = atoi(payload);
        printf("publisher: config — delay=%dms\n", cfg_delay_ms);

    } else if (strcmp(topic, "request/messagesize") == 0) {
        cfg_msg_size = atoi(payload);
        printf("publisher: config — msg_size=%d\n", cfg_msg_size);
...
int main(int argc, char *argv[]) {
    ...
    MQTTClient_create(&g_client, broker, CLIENT_ID,
